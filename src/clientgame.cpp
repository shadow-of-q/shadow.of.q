...
namespace game
{
  ...
  void mode(int n) { client::addmsg(1, 2, SV_GAMEMODE, nextmode = n); }
  ...
  void respawn()
  {
    ...
    { 
      ...
      if (m_sp) { nextmode = gamemode; client::changemap(clientmap); return; }    // if we die in SP we try the same map again
      respawnself();
    }
  }
  ...
  void updateworld(int millis)        // main game update loop
  {
    ...
    {     
      ...
      if (sleepwait && lastmillis>sleepwait) { sleepwait = 0; cmd::execute(sleepcmd); }
      physicsframe();
      checkquad(curtime);
      ...
      if (!demoplayback)
      {
        if (client::getclientnum()>=0) shoot(player1, worldpos);     // only shoot when connected to server
        client::gets2c();           // do this first, so we have most accurate information when our player moves
      }
      otherplayers();
      if (!demoplayback)
      {
        ...
        client::c2sinfo(player1);   // do this last, to reduce the effective frame lag
      }
    }
    lastmillis = millis;
  }
...
#define dir(name,v,d,s,os) void name(bool isdown) { player1->s = isdown; player1->v = isdown ? d : (player1->os ? -(d) : 0); player1->lastmove = lastmillis; }
  ...
  {
    if (client::netmapstart() && m_sp) { gamemode = 0; console::out("coop sp not supported yet"); }
    sleepwait = 0;
    monsterclear();
    projreset(); 
    spawncycle = -1;

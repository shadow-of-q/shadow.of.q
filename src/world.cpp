...
// detect those cases where a higher mip solid has a visible wall next to
// lower mip cubes (used for wall rendering below)
...
void render_seg_new(float vx, float vy, float vh, int mip, int x, int y, int xs, int ys)
{
  ...
  for (int ox = x; ox<xs; ox++) /* first collect occlusion information for this block */
...
#define dc(x) s->ceil+(x->vdelta/4.0f)
  ...
  // facilitate dynamic stripify) for each we skip occluded cubes (occlusion
  // at higher mip levels is a big time saver!).  during the first loop (ceil)
  // we collect cubes that lie within the lower mip rect and are also
  // deferred, and render them recursively. Anything left (perfect mips and
  // higher lods) we render here.
...
void render(float vx, float vy, float vh, int yaw, int pitch, float fov, int w, int h)
{
  ...
  if (cdist<7) {  /* hack to avoid popup at high fovs at 45 yaw */
    min_lod = max(min_lod, (int)(MIN_LOD+(10-cdist)/1.0f*widef));
    widef = 1.0f;
  }
  lod = MAX_LOD;
